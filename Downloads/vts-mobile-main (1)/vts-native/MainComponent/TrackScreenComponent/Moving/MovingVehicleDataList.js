...
import { MeterToKm, setOdometer, setDateFunction } from '../../../constants/UnitConvert';
...
const MovingVehicleDataList = ({ socketData, nullMapListFilter, setMapIdVehicle, ScrollRef, setSelectedData }) => {
      ...
      <View style={{ paddingBottom: 300 }}>
        {uniqByKeepLast(nullMapListFilter, it => it?.device?.id)?.map((item, index) => (
          <View key={index} style={{ backgroundColor: '#ffffff', marginTop: 5, borderRadius: 5, padding: 10, }}>
                  ...
                  <Center style={{ borderWidth: 1, borderColor: '#67748E', width: 30, height: 30, borderRadius: 50 }}>
                    {item?.position === null ?
                      ...
                      item?.position.attributes.sat === undefined ?
                        <MaterialCommunityIcons name='crosshairs-gps' size={18} color={'#67748E'} /> :
                        <MaterialCommunityIcons name='crosshairs-gps' size={18} color={socketData?.attributes?.sat < 11 ? '#67748E' : '#63CE78'} />
                    }
                  </Center>
                  <Box style={{ paddingLeft: 5 }}>
                ...
                <View style={{ display: 'flex', alignItems: 'center', flexDirection: 'row', paddingTop: 20 }}>
                  ...
                  <Box style={{ paddingLeft: 5 }}>
                    ...
                    {item?.position === null ?
                      ...
                      :
                      <Text style={{ fontSize: 10, color: '#252F40', fontFamily: 'OpenSans-SemiBold' }}>{socketData?.attributes?.battery === undefined ? '0' : socketData?.attributes?.battery === null ? 0 : (socketData?.attributes?.battery)?.toFixed(2)} V</Text>
                    }
                  </Box>
                </View>
              </Col>
                    ...
                    <Row>
                      <Col xs={6} sm={6} md={6} lg={6}>
                        <View style={{ marginTop: 20 }}>
                          ...
                          <Text style={{ fontFamily: 'OpenSans-Regular', fontSize: 11, color: '#252F40' }}>{socketData?.latitude}</Text>
                        </View>
                      </Col>
                      <Col xs={6} sm={6} md={6} lg={6}>
                        <View style={{ marginTop: 20 }}>
                          <Text style={{ fontFamily: 'OpenSans-SemiBold', fontSize: 12, color: '#252F40' }}>Engine</Text>
                          <Text style={{ fontFamily: 'OpenSans-Regular', fontSize: 11, color: '#252F40' }}>{item?.position === null ? '-' : socketData?.attributes?.ignition === null ? '-' : socketData?.attributes?.ignition === true ? 'ON' : 'OFF'}</Text>
                        </View>
                      </Col>
                      <Col xs={6} sm={6} md={6} lg={6}>
                        <View style={{ marginTop: 20 }}>
                          <Text style={{ fontFamily: 'OpenSans-SemiBold', fontSize: 12, color: '#252F40' }}>Longitude</Text>
                          <Text style={{ fontFamily: 'OpenSans-Regular', fontSize: 11, color: '#252F40' }}>{socketData?.longitude}</Text>
                        </View>
                      </Col>
                      ...
                      <Col xs={6} sm={6} md={6} lg={6}>
                        <View style={{ marginTop: 20 }}>
                          ...
                          <Text style={{ fontFamily: 'OpenSans-Regular', fontSize: 11, color: '#252F40' }}>{socketData?.speed.toFixed(2)} Km/hr</Text>{/* addLeadingZeros(item?.position?.speed) */}
                        </View>
                      </Col>
                      <Col xs={6} sm={6} md={6} lg={6}>
                        <View style={{ marginTop: 20 }}>
                      ...
                      <Col xs={6} sm={6} md={6} lg={6}>
                        <View style={{ marginTop: 20 }}>
                          ...
                          <Text style={{ fontFamily: 'OpenSans-Regular', fontSize: 11, color: '#252F40' }}>{item?.position === null ? '-' : socketData?.attributes?.totalDistance === null ? '-' : setOdometer(socketData?.attributes?.totalDistance)}</Text>
                        </View>
                      </Col>
                      <Col xs={6} sm={6} md={6} lg={6}>
                        <View style={{ marginTop: 20 }}>
                      ...
                      <View style={{ display: 'flex', alignItems: 'center', flexDirection: 'row' }}>
                        ...
                        <Box style={{ paddingLeft: 5 }}>
                          ...
                          {item?.position === null ?
                            <Text style={{ fontSize: 10, color: '#252F40', fontFamily: 'OpenSans-SemiBold' }}>-</Text>
                            :
                            <Text style={{ fontSize: 10, color: '#252F40', fontFamily: 'OpenSans-SemiBold' }}>{socketData?.attributes?.power === undefined ? '0' : socketData?.attributes?.power === null ? 0 : (socketData?.attributes?.power)?.toFixed(2)} V</Text>
                          }
                        </Box>
                      </View>
                    </Col>
            ...
            <Pressable onPress={() => expanded === item?.device.id ? handlePress({ id: 0, lat: item?.position === null ? '-' : socketData?.latitude, long: item?.position === null ? '-' : socketData?.longitude }) : handlePress({ id: item?.device.id, lat: item?.position === null ? '-' : socketData?.latitude, long: item?.position === null ? '-' : socketData?.longitude })} style={{ display: 'flex', alignItems: 'center', flexDirection: 'row', justifyContent: 'center' }}>
              {expanded === item?.device.id ?
                <AntDesign onPress={() => handlePress({ id: 0, lat: item?.position === null ? '-' : socketData?.latitude, long: item?.position === null ? '-' : socketData?.longitude })} name={"minuscircleo"} size={18} color={'blue'} />
                :
                <AntDesign onPress={() => handlePress({ id: item?.device.id, lat: item?.position === null ? '-' : socketData?.latitude, long: item?.position === null ? '-' : socketData?.longitude })} name={"pluscircleo"} size={18} color={'blue'} />
              }
              {expanded === item?.device.id ?
...
export default MovingVehicleDataList;
