...
const ListHeaderComponent = ({ valueFilter, entityDataCount, navigation, }) => { //handleClick
  ...
  const exportDataToExcel = async () => {
    ...
    setLoader(true)
    await ApiRequestAuthorizationHook.get(`/search/device?search_key=${valueFilter}`, { signal: signal })
      .then(function (ress) {
        if (ress.status === 200) {
  ...
  return (
    <Fragment>
      ...
      <View style={{ /* marginTop: 10, */ paddingTop: 10, paddingBottom: 10, paddingHorizontal: 15, backgroundColor: '#ebebfd', }}>
