...
const EntityDataList = ({ expanded, handlePress, navigation, item, }) => {
          ...
          <View style={{ backgroundColor: '#FFFFFF', borderBottomLeftRadius: 10, borderBottomRightRadius: 10, paddingBottom: 20 }}>
            <View style={{ marginHorizontal: 10 }}>
              ...
              {item?.otpverified === true ?
                ...
                <View>
                  {isEdit ?
                    <Pressable onPress={() => navigation.navigate({ name: 'EditEntityScreen', params: { entity_id: item?.id } })} style={{ backgroundColor: '#7474f5', paddingTop: 8, paddingBottom: 8, borderRadius: 5, width: 100, marginTop: 10, display: 'flex', alignItems: 'center', flexDirection: 'row', justifyContent: 'center' }}>
