...
import ReportedViolationList from '../MiscellaneousReportsList/ReportedViolationList';

const ReportedViolation = ({ setToggleReport }) => {
  ...
  const reques = async () => {
    ...
    await ApiRequestAuthorizationHook.get(`/reports/permit/violations/reported/?${data}&currentPage=${page}&pageSize=${pageSize}`, { signal: signal })
  ...
  const onDwonLoad = () => {
    ...
    RNFetchBlob
      ...
      .fetch('GET', `${ApiDownload.baseURL}/reports/permit/violations/reported/export?${onDwonloadData}`, {
    ...
    <Fragment>
      ...
      <View>
        <>
          {toggleComponent ?
            <>
              <ReportedViolationList fetchMoreData={fetchMoreData} loadMore={loadMore} loader={loader} listData={vehicle_list} />
            </>
            :
            <>
...
export default ReportedViolation