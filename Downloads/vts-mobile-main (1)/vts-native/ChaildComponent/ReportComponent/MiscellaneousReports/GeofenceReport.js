...
import GeofenceReportList from '../MiscellaneousReportsList/GeofenceReportList';

const GeofenceReport = ({ setToggleReport }) => {
  ...
  const [geofenceData, setGetGeofenceData] = useState([])
  ...
  const [geofenceId, setGeofenceId] = useState(false);
  ...
  const onCHangeGeofenceId = (Text) => {
    if (Text.trim().length >= 0) {
      setValue({ ...value, GeofenceId: Text });
      setGeofenceId(false);
    } else {
      setValue({ ...value, GeofenceId: Text });
      setGeofenceId(false)
  ...
  const validate = () => {
    ...
    if (checkStringNullEmpty(value?.GeofenceId)) {
      validation += '<li>Enter Your Confrim Password</li>';
      setGeofenceId(true)
    }
  };
  ...
  const reques = async () => {
    ...
    await ApiRequestAuthorizationHook.get(`/reports/geofences/?${data}&currentPage=${page}&pageSize=${pageSize}`, { signal: signal })
  ...
  const getGeofenceData = async () => {
    ...
    await ApiRequestAuthorizationHook.get(`/geofences`, { signal: signal })
      .then(function (ress) {
        if (ress?.status === 200) {
          setGetGeofenceData(ress?.data?.data)
          setLoader(false)
        }
      })
      .catch(function (err) {
        console.log('get geofence data', err)
        setLoader(false)
      })
  ...
  const searchGeofenceData = geofenceData?.filter(
    GeofenceItem => {
      return (
        GeofenceItem
          .name
  ...
  var path = RNFetchBlob.fs.dirs.DownloadDir + `/geofences_report` + Math.floor(dateDwonLoad?.getTime() + dateDwonLoad?.getSeconds() / 2) + '.xlsx';
  ...
  const onDwonLoad = () => {
    ...
    RNFetchBlob
      ...
      .fetch('GET', `${ApiDownload.baseURL}/reports/geofences/export?${onDwonloadData}`, {
    ...
    <Fragment>
      ...
      {toggleComponent ?
        <View style={{ backgroundColor: '#ebebfd', }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginHorizontal: 15, paddingBottom: 10 }}>
            ...
            <Pressable onPress={() => { setToggleComponent(false); setToggleReport(false); setApiLoad(false), setValue({ ...value, DateFrom: '', DateTo: '', GeofenceId: '' }); setRangeTime({ ...rangeTime, TimeForm: '', TimeTo: '' }); setVehicle_list([]); setPage(1); setFromDate(new Date(d)); setToDate(new Date()) }} style={{ flexDirection: 'row', alignItems: 'center', paddingRight: 10 }}>
      ...
      <View>
        <>
          {toggleComponent ?
            <>
              <GeofenceReportList value={value} fetchMoreData={fetchMoreData} loadMore={loadMore} loader={loader} listData={vehicle_list} />
            </>
            :
            <>
              <View style={{ backgroundColor: '#ffffff', /* marginHorizontal: 15, */ marginTop: 20, borderRadius: 5, /* padding: 10, */ paddingBottom: 20 }}>
                <View style={{ paddingTop: 12 }}>
                  <Text style={{ color: '#000000', fontSize: 12, fontFamily: 'OpenSans-SemiBold', paddingBottom: 4 }} >Geofence</Text>
                  <Select onOpen={() => searchGeofenceData?.length === 0 ? getGeofenceData() : null} style={{ color: '#252F40', fontSize: 14, fontFamily: 'OpenSans-Regular', height: 40 }} borderWidth={0} selectedValue={value?.GeofenceId} placeholderTextColor={'#7D8EAB'} minWidth="200" accessibilityLabel="Select" placeholder="Select" _selectedItem={{
                    bg: "#f4f4fd",
                    ...
                    onValueChange={itemValue => onCHangeGeofenceId(itemValue)}
                    _actionSheetBody={{
                      ListHeaderComponent:
                        <View>
                          {loader === true ?
                  ...
                  >
                    {loader ?
                      ...
                      :
                      searchGeofenceData?.map((value, id) => (
