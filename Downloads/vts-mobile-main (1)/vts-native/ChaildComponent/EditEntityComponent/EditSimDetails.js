...
import { View, Text, Pressable, TouchableOpacity, ActivityIndicator, BackHandler } from 'react-native';
...
const EditSimDetails = ({ setDeviceDetailsColor, setSimDetailsColor, setVehicleDetailsColor, setPermitHolderColor, setDeviceDetailsView, setSimDetailsView, setVehicleDetailsView, setPermitHolderView, }) => {
      ...
      await AsyncStorage.getItem('DeviceDetails')
        .then(function (res) {
          ...
          setValue({
            ...
            Sim1Expiry: JSONObj?.sim1_expiry === null ? '' : moment(JSONObj?.sim1_expiry).format("DD/MM/YYYY"), // `${new Date(JSONObj?.sim1_expiry).getFullYear()}-${(new Date(JSONObj?.sim1_expiry).getMonth() + 1)?.toString()?.padStart(2, "0")}-${new Date(JSONObj?.sim1_expiry).getDate()?.toString()?.padStart(2, "0")}`,
            SimNumber2: JSONObj?.simno2,
            Network2: JSONObj?.network2,
            Sim2Expiry: JSONObj?.sim2_expiry === null ? '' : moment(JSONObj?.sim2_expiry).format("DD/MM/YYYY"), // `${new Date(JSONObj?.sim2_expiry).getFullYear()}-${(new Date(JSONObj?.sim2_expiry).getMonth() + 1)?.toString()?.padStart(2, "0")}-${new Date(JSONObj?.sim2_expiry).getDate()?.toString()?.padStart(2, "0")}`,
            iccId: JSONObj?.iccid,
      ...
      <View style={{ backgroundColor: '#ffffff', marginHorizontal: 15, marginTop: 20, borderRadius: 5, padding: 10, }}>
        ...
        <View>
          ...
          <View style={{ paddingTop: 12 }}>
            ...
            <View style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', backgroundColor: '#f4f4fd', borderRadius: 5, height: 40, marginTop: 5 }}>
              <Input value={value.SimNumber1} keyboardType='number-pad' maxLength={13} onChangeText={(Text) => onChangeSimNumber1(Text)} style={{ color: '#252F40', fontSize: 14, fontFamily: 'OpenSans-Regular', height: 40 }} variant={'unstyled'} placeholderTextColor={'#7D8EAB'} placeholder="Enter Number" w="100%" />
            </View>
            {simNumber1 ?
              <View>
        ...
        <View style={{ paddingTop: 20 }}>
          ...
          <View style={{ paddingTop: 12 }}>
            ...
            <View style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', backgroundColor: '#f4f4fd', borderRadius: 5, height: 40, marginTop: 5 }}>
              <Input value={value.SimNumber2} keyboardType='number-pad' maxLength={13} onChangeText={(Text) => onChangeSimNumber2(Text)} style={{ color: '#252F40', fontSize: 14, fontFamily: 'OpenSans-Regular', height: 40 }} variant={'unstyled'} placeholderTextColor={'#7D8EAB'} placeholder="Enter Number" w="100%" />
            </View>
            {simNumber2 ?
              <View>
