...
import { StyleSheet, View, Pressable, Dimensions, Modal, LayoutAnimation, UIManager, Platform, ActivityIndicator, Alert } from 'react-native';
...
const CreateGeofence = ({ navigation }) => {
  const mapRef = useRef()
  ...
  var geofenceObj = JSON.stringify({
    ...
    area: value.user_area_type === "CIRCLE" ? `${value.user_area_type}(${value.user_latitude} ${value.user_longitude},${value.radius})` : value.user_area_type === "POINTER" ? `${value.user_area_type}(${value.user_latitude} ${value.user_longitude})` : value.user_area_type === "POLYGON" ? `${value.user_area_type}(${value.user_latitude} ${value.user_longitude})` : null,
    createdon: new Date()
  })
  ...
  const onSubmit = async () => {
    ...
    if (validation === '') {
      setLoader(true)
      await ApiRequestAuthorizationHook.post(`/geofences`, geofenceObj)
        .then(function (ress) {
          if (ress.status === 200) {
            setLoader(false)
            setCustomModal({ ...customModal, modal: true, message: 'Geofence Created Successfully', status: 1 })
          }
        })
        .catch(function (err) {
          console.log('create geofence', err)
          setLoader(false)
          setCustomModal({ ...customModal, modal: true, message: 'Geofence Created failed', status: 0 })
        })
    } else {
      showAlert();
    }
  ...
  return (
    <Fragment>
      ...
      <View style={{ marginBottom: 20, marginHorizontal: 15 }}>
        <View style={{ marginTop: 10, backgroundColor: Colors.white, elevation: 5, /* height: 600, */ borderRadius: 5, paddingHorizontal: 10, paddingBottom: 15 }}>
            ...
            <Row style={{ display: 'flex', alignItems: 'center', flexDirection: 'row', justifyContent: 'center' }}>
              ...
              <Col xs={6} sm={6} md={6} lg={6}>
                <Pressable onPress={() => onSubmit()} style={{ backgroundColor: '#4646F2', height: 40, justifyContent: 'center', marginTop: 5, borderRadius: 8 }}>
                  <Text style={{ color: Colors.white, textAlign: 'center', fontFamily: 'OpenSans-SemiBold', fontSize: 14 }}>Create</Text>
                </Pressable>
              </Col>
            </Row>
          </View>
