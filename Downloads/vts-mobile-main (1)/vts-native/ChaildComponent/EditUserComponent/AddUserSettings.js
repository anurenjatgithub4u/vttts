    ...
    await ApiRequestAuthorizationHook.get(`/users/${location?.params?.user_id}`, { signal: signal })
      .then(function (res) {
        if (res?.status === 200) {
          setValue({
            ...value,
            MapLayer: res?.data?.map,
            Latitude: res?.data?.latitude,
            Longitude: res?.data?.longitude,
            Zoom: res?.data?.zoom,
            CoordinateFormate: res?.data?.coordinateFormat,
            Layer: res?.data?.poiLayer,
            Hour_24_format: res?.data?.twelveHourFormat
          })
          validLatitude(res?.data?.latitude)
          validLongitude(res?.data?.longitude)
        }
      })
      .catch(function (err) {
        console.log(err);
  ...
  return (
    <Fragment>
      <Modal visible={loader} transparent={true}>
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#0000ff5e' }}>
      ...
      <View style={{ backgroundColor: '#ffffff', marginHorizontal: 15, marginTop: 20, borderRadius: 5, padding: 10 }}>
        <View>
          ...
          <View style={{ paddingTop: 12 }}>
            ...
            <Select placeholderTextColor={'#7D8EAB'} style={{ height: 40, backgroundColor: '#f4f4fd', fontFamily: 'OpenSans-Regular', fontSize: 14, color: '#252F40' }} borderWidth={0} selectedValue={value.MapLayer} minWidth="200" accessibilityLabel="Select" placeholder="Custom" _selectedItem={{
              bg: "#f4f4fd",
          ...
          <View style={{ paddingTop: 12 }}>
            ...
            <Input placeholderTextColor={'#7D8EAB'} value={value.Latitude.toString()} keyboardType='numeric' onChangeText={(Text) => onChangeLatitude(Text)} variant={'unstyled'} w={'100%'} placeholder='Custom' style={{ color: '#252F40', fontSize: 14, fontFamily: 'OpenSans-Regular', marginTop: 5, backgroundColor: '#f4f4fd', borderRadius: 5, height: 40, }} />
            {latitude ?
              <View>
          ...
          <View style={{ paddingTop: 12 }}>
            ...
            <Input placeholderTextColor={'#7D8EAB'} value={value.Longitude.toString()} keyboardType='numeric' onChangeText={(Text) => onChangeLongitude(Text)} variant={'unstyled'} w={'100%'} placeholder='Custom' style={{ color: '#252F40', fontSize: 14, fontFamily: 'OpenSans-Regular', marginTop: 5, backgroundColor: '#f4f4fd', borderRadius: 5, height: 40, }} />
            {longitude ?
              <View>
          ...
          <View style={{ paddingTop: 12 }}>
            ...
            <Input placeholderTextColor={'#7D8EAB'} keyboardType={'number-pad'} value={value.Zoom.toString()} onChangeText={(Text) => onChangeZoom(Text)} variant={'unstyled'} w={'100%'} placeholder='Enter' style={{ color: '#252F40', fontSize: 14, fontFamily: 'OpenSans-Regular', marginTop: 5, backgroundColor: '#f4f4fd', borderRadius: 5, height: 40, }} />
            {zoom ?
              <View>
          ...
          <View style={{ paddingTop: 12 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              />
              <Text style={{ color: 'gray', fontSize: 12, fontFamily: 'OpenSans-SemiBold', }} >24 Hour format</Text>
            </View>
            {/* {layer ?
              <View>
