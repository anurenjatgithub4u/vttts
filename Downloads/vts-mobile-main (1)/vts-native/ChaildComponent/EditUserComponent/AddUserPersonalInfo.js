...
const AddUserPersonalInfo = ({ setAddUserPersonalInfoView, setAddUserSettingsView, setAddUserPersonalInfoActive, setAddUserSettingsActive, setAddUserPermissionView, setAddUserPermissionActive }) => {
  ...
  const getRoleData = async () => {
    await ApiRequestAuthorizationHook.get(`/user_roles`)
      .then(function (res) {
        if (res.status === 200) {
  ...
  const roleName = roleData.filter(itemInArray => itemInArray?.id === value?.Role)[0]?.rolename
  const selectRoleName = roleName === undefined ? 'Select' : roleName
      ...
      <View style={{ backgroundColor: '#ffffff', marginHorizontal: 15, marginTop: 20, borderRadius: 5, padding: 10 }}>
        <View>
          ...
          <View style={{ paddingTop: 12 }}>
            ...
            <Select onOpen={() => roleData?.length === 0 ? getRoleData() : null} placeholderTextColor={roleName === undefined ? '#7D8EAB' : '#252F40'} style={{ height: 40, backgroundColor: '#f4f4fd', fontFamily: 'OpenSans-Regular', fontSize: 14, color: '#252F40' }} borderWidth={0} defaultValue="" selectedValue={value?.Role} minWidth="200" accessibilityLabel="Select" placeholder={selectRoleName} _selectedItem={{
              bg: "#f4f4fd",
