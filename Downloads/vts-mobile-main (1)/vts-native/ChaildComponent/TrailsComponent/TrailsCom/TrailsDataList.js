...
const TrailsDataList = ({ CurrentPoint, sliderHandler, totalLenght, setIsAnimate, setIsSpeed, isSpeed, isAnimate, onToggle, onStop, onStart, onPause, itemObject, }) => {
  ...
  return (
    <Fragment>
      ...
      <View style={{
        ...
        <TrailsListHeader itemObject={itemObject} />

        <TrailsChart itemObject={itemObject} />

        <View style={{ display: 'flex', alignItems: 'center', flexDirection: 'row', justifyContent: 'center', marginTop: 50 }}>
          ...
          {isAnimate ?
            <TouchableOpacity onPress={() => { onPause(); onToggle() }} style={{ alignItems: 'center', justifyContent: 'center' }}>
              <AntDesign name={'pausecircleo'} size={28} color={'#252F40'} />
            </TouchableOpacity>
            :
            <TouchableOpacity onPress={() => { onStart(); onToggle() }} style={{ alignItems: 'center', justifyContent: 'center' }}>
              <AntDesign name={'playcircleo'} size={28} color={'#252F40'} />
            </TouchableOpacity>
          }
        ...
        <Pressable style={{ display: 'flex', alignItems: 'center', flexDirection: 'row', marginTop: 15 }}>
          <Pressable style={{ display: 'flex', alignItems: 'center', flexDirection: 'row' }}>
            ...
            <Text style={{ color: 'blue', fontFamily: 'OpenSans-SemiBold', fontSize: 14, marginLeft: 10 }}>{itemObject?.deviceName === null ? '-' : itemObject?.deviceName}</Text>
            <View style={{ flexGrow: 1 }} />
          </Pressable>
        </Pressable>
