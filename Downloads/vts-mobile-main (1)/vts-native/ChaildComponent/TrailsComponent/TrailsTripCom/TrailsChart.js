...
const TrailsChart = ({ routeGraph, itemObject }) => {
  ...
  useEffect(() => {
    if (routeGraph) {
      if (routeGraph?.length > 0) {
        ...
        let devicetimearr = [];
        let dayarr = [];
        routeGraph?.map((value, index) => {
          speedarr.push(value?.speed === undefined || null ? 0 : value?.speed);
          batteryarr.push(value?.attributes?.battery === undefined || null ? 0 : value?.attributes?.battery);
