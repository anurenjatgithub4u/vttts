...
const MapViewCom = ComponentLoadable(lazy(() => import('./TrailsTripCom/MapViewCom')));
...
const TrailsTrip = ({ trailsTripMapShow, setTrailsTripMapShow, itemObject, setItemObject, trailsData, setTrailsData, value, setValue, fromDate, setFromDate, toDate, setToDate, onResetFunc, isDays, setIsDays, }) => {
  ...
  const onSubmit = async () => {
    ...
    if (validation === '') {
      if (isDays === false) {
        setApiLoading(true)
        await ApiRequestAuthorizationHook.get(`/trips/trails?${data}`, { signal: signal })
          .then(function (response) {
            if (response.status === 200) {
              setTrailsData(response?.data?.data);
              setTrailsTripMapShow(true);
            } else {
    ...
    <>
      ...
      <View style={{ paddingBottom: 40 }}>
        <Fragment>
          ...
          {trailsTripMapShow ?
            <MapViewCom onResetFunc={onResetFunc} itemObject={itemObject} setItemObject={setItemObject} trailsData={trailsData} />
            :
            <>
              <View style={{ backgroundColor: '#ffffff', marginHorizontal: 15, marginTop: 20, borderRadius: 5, padding: 10, paddingBottom: 20, }}>
                <View style={{ paddingTop: 12 }}>
                  ...
                  <Select onOpen={() => searchDeviceData.length === 0 ? getDataDevice() : null} style={{ color: '#252F40', fontSize: 14, fontFamily: 'OpenSans-Regular', height: 40 }} placeholderTextColor={'#7D8EAB'} borderWidth={0} selectedValue={value.Device} minWidth="200" accessibilityLabel="Select" placeholder="Select" _selectedItem={{
                    bg: "#f4f4fd",
