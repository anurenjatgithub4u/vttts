...
const PermitHolder = ({ setDeviceDetailsColor, setSimDetailsColor, setVehicleDetailsColor, setPermitHolderColor, setDeviceDetailsView, setSimDetailsView, setVehicleDetailsView, setPermitHolderView, navigation, }) => {
  ...
  const DeviceDetails = async () => {
    ...
    // console.log('DeviceDetails_result', obj)
  ...
  const SimDetails = async () => {
    ...
    // console.log('SimDetails_result', SimDetails_result)
    setSimDetails(obj)
  }
  ...
  var data = JSON.stringify({
    ...
    name: vehicleDetails?.DeviceNumber,
    ...
    engineno: vehicleDetails?.EngineN,
  ...
  const onNexnt = async () => {
    ...
    if (validation === '') {
      if (holderNumber === false && driverNumber === false) {
        setLoader(true)
        await ApiRequestAuthorizationHook.post(`/devices`, data)
          .then(function (ress) {
            if (ress.status === 200) {
              setLoader(false);
              setCustomModal({ ...customModal, modal: true, message: 'Successfully Created Entity', status: 1 })
            }
          })
          .catch(function (err) {
            console.log('Entity Created', err?.response)
            setCustomModal({ ...customModal, modal: true, message: 'Unable to Create Entity', status: 0 })
          })
          .finally(() => {
            setLoader(false)
          })
    ...
    <Fragment>
      ...
      <View style={{ backgroundColor: '#ffffff', marginHorizontal: 15, marginTop: 20, borderRadius: 5, padding: 10 }}>
        ...
        <View style={{ paddingTop: 12 }}>
          ...
          {point_Of_Contact_Name ?
            <View>
              <Text style={{ color: 'red', fontSize: 11, fontFamily: 'OpenSans-Regular' }}>Driver Name Name is required</Text>
            </View>
            : null
          }
        </View>
        <View style={{ paddingTop: 12 }}>
          ...
          <View style={{ backgroundColor: '#f4f4fd', borderRadius: 5, height: 40, marginTop: 5 }}>
            <Input keyboardType='number-pad' maxLength={10} value={value?.Point_Of_Contact_Number} onChangeText={(Text) => onChangePoint_Of_Contact_Number(Text)} variant={'unstyled'} placeholderTextColor={'#7D8EAB'} placeholder='Driver Number' style={{ color: '#252F40', fontSize: 14, fontFamily: 'OpenSans-Regular', height: 40 }} />
          </View>
          {point_Of_Contact_Number ?
            <View>
