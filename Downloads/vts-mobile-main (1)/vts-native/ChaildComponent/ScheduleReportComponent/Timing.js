...
const Timing = ({ setGeneralinfoActive, setTimingActive, setGeneralinfoView, setTimingView, navigation }) => {
  ...
  const onNexnt = async () => {
    ...
    if (validation === '') {
      setLoader(true);
      await ApiRequestAuthorizationHook.post(`/report_schedules`, reportObj)
        .then(function (ress) {
          if (ress.status === 200) {
            setLoader(false);
            setCustomModal({ ...customModal, modal: true, message: 'Reports Schedule Created Successfully', status: 1 })
          }
        })
        .catch(function (err) {
          console.log('Reports Schedule', err)
          setLoader(false);
          setCustomModal({ ...customModal, modal: true, message: 'Failed to create Reports Schedule', status: 0 })
        })
    }
  }
    ...
    <Fragment>
      ...
      <View style={{ backgroundColor: '#ffffff', marginHorizontal: 15, marginTop: 20, borderRadius: 5, padding: 10 }}>
        ...
        <View style={{ paddingTop: 12 }}>
          ...
          <Select placeholderTextColor={'#67748E'} style={{ color: '#252F40', fontSize: 14, fontFamily: 'OpenSans-Regular', height: 40 }} borderWidth={0} selectedValue={value.SelectFormat} minWidth="200" accessibilityLabel="Select Foramt" placeholder="Select Format" _selectedItem={{
            bg: "#f4f4fd",
      ...
      <Pressable onPress={() => onNexnt()} style={{ backgroundColor: '#4646f2', paddingTop: 10, paddingBottom: 10, borderRadius: 5, marginTop: 15, marginHorizontal: 15, marginBottom: 20 }}>
        <Text style={{ textAlign: 'center', color: '#ffffff', fontFamily: 'OpenSans-SemiBold', fontSize: 14 }}>Create Schedule</Text>
      </Pressable>
    </Fragment>
  )
}
