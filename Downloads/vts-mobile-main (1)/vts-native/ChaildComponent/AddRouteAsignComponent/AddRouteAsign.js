...
import { View, Text, Pressable, ActivityIndicator, Modal, ScrollView, Dimensions, StyleSheet, StatusBar } from 'react-native';
import { Actionsheet, useDisclose, } from 'native-base';
import { Select, CheckIcon, Input, TextArea, Divider, Icon } from 'native-base';
...
const AddRouteAsign = () => {
  ...
  const onSubmit = async () => {
    ...
    if (validationuser === '') {
      setLoader(true)
      await ApiRequestAuthorizationHook.post(`/trips`, asigndata)
        .then(function (ress) {
          if (ress.status === 200) {
            console.log(ress)
            setLoader(false)
            setCustomModal({ ...customModal, modal: true, message: 'Route Assign Successfully', status: 1 })
          }
        })
        .catch(function (err) {
          console.log('asign trips', err)
          setLoader(false)
          setCustomModal({ ...customModal, modal: true, message: 'Route Assign failed', status: 0 })
        })
    }
  }
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={{ backgroundColor: '#ffffff', borderRadius: 4, padding: 10, marginBottom: 20, marginHorizontal: 20, marginTop: 20, elevation: 12 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
            <Text style={{ color: '#252F40', fontSize: 14, fontFamily: 'OpenSans-Bold' }}>Assign new Route</Text>
          ...
          <View style={{ marginTop: 15 }}>
            ...
            <View style={{ backgroundColor: '#e2e2fd', borderRadius: 5, height: 40, }}>
              <Select onOpen={() => searchRouteData?.length === 0 ? getDataRoute() : null} placeholderTextColor={'#7D8EAB'} borderWidth={0} style={{ color: '#252F40', fontFamily: 'OpenSans-Regular', fontSize: 14, height: 40 }} selectedValue={value?.RouteName} minWidth="200" placeholder="Select" _selectedItem={{
          ...
          <View style={{ marginTop: 15 }}>
            ...
            <View style={{ backgroundColor: '#e2e2fd', borderRadius: 5, height: 40, }}>
              <Select onOpen={() => vehicleData?.length === 0 ? getDataVehicle() : null} placeholderTextColor={'#7D8EAB'} borderWidth={0} style={{ color: '#252F40', fontFamily: 'OpenSans-Regular', fontSize: 14, height: 40 }} selectedValue={value?.VehicleName} minWidth="200" placeholder="Select" _selectedItem={{
                endIcon: <CheckIcon size="5" />
              }}
          ...
          <View style={{ marginTop: 15 }}>
            ...
            <View style={{ backgroundColor: '#e2e2fd', borderRadius: 5, height: 40, }}>
              <Select onOpen={() => deriverData?.length === 0 ? getDataDrivers() : null} placeholderTextColor={'#7D8EAB'} borderWidth={0} style={{ color: '#252F40', fontFamily: 'OpenSans-Regular', fontSize: 14, height: 40 }} selectedValue={value?.DriverName} minWidth="200" placeholder="Select" _selectedItem={{
                endIcon: <CheckIcon size="5" />
              }}
          ...
          <View style={{ marginTop: 15 }}>
            ...
            <View style={{ backgroundColor: '#e2e2fd', borderRadius: 5, height: 40, }}>
              <Select placeholderTextColor={'#7D8EAB'} borderWidth={0} style={{ color: '#252F40', fontFamily: 'OpenSans-Regular', fontSize: 14, height: 40 }} selectedValue={value?.ClosureTypeField} placeholder={'Select'} minWidth="200" _selectedItem={{
                endIcon: <CheckIcon size="5" />
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingTop: 20, paddingBottom: 20, paddingHorizontal: 20 }}>
          <Pressable onPress={onSubmit} style={{ justifyContent: 'center', alignItems: 'center', backgroundColor: 'blue', height: 40, width: '100%', borderRadius: 5 }}>
            <Text style={{ textAlign: 'center', color: '#FFFFFF', fontFamily: 'OpenSans-Regular', fontSize: 14 }}>Submit</Text>
          </Pressable>
        </View>
      </ScrollView>
